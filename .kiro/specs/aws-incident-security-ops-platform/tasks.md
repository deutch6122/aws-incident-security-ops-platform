...
- [x] 20. 最終 Checkpoint — 全体の検証
  - Ensure all tests pass, ask the user if questions arise.
  - _Verified in Task 20: Tasks 1–19 完了確認（未完了は Task 20 のみ、主要タスク 7–19 に _Implemented/_Verified 補足行あり、Dependency Graph と整合）。指定コマンド単位で全テスト green（infra 17 module 一括 201 / db migrations 8＋1 skip / scripts 50 / backend-api 55＋compileall OK / eks-workers 42＋3 skip＋compileall OK / portal-lambda 21＋compileall OK / portal-frontend 13）、deploy 3 スクリプト bash -n OK。skip は db=psycopg/testcontainers 未導入 1、eks-workers=moto 未導入 3 のみで実装未完了 skip は 0 件（Property 10 は skip せず pass、A→B fake ベース pass、moto/testcontainers は未実行事項として記録済み）。Property 1〜11 は各単一テストで PBT タグ付き実装（1/3/5 backend, 2/4/6 backend, 7/8/9 eks-workers, 10 portal-lambda, 11 dev root）。Secret/credential/実 ARN 非混入（git 管理に .venv/.env/tfstate/tfvars.local/pem/credentials・cache 類なし、account ID は 123456789012/111122223333/000000000000 の AWS ドキュメント用 placeholder のみ、AKIA/JWT/DB URL/実ドメイン/実 Bearer なし、Bearer はヘッダ構造記述と CloudFront 転送ヘッダ assertion のみ、frontend は REPLACE_WITH_* のみ）。Product_A/B 分離維持（lambda IAM は DynamoDB 3 テーブル読取＋page_view_logs のみ書込・Product_A 権限なし、portal-lambda は Product_A 不参照を明記、A→B は link_summary_to_portal を monthly_summary_cronjob からのみ起動・write-only port・非機微のみ・B→A なし）。IaC は remote state/pipeline(bootstrap)＋network/ecr/aurora/alb/ecs/eks/messaging/logging/dynamodb/s3-portal/cloudfront/cognito/apigateway/lambda/monitoring/(waf/iam) 存在、dev root は network/ecr/aurora のみ配線・他は依存値未確定で意図的未配線（README 表と各 module README に理由・後続配線値を明記、CloudFront/WAF us-east-1 provider alias・S3 OAC policy と distribution ARN 配線順・API GW→Lambda aws_lambda_permission も明記）。App/script/docs 揃い（backend 4 router、eks 3 entrypoint、portal 5 画面、docs 6＋補足、deploy dry-run 既定＋--execute、seed dry-run 既定、runbook/デモ/mermaid/撤去手順/コスト・セキュリティ注意）。git working tree clean。ブロッカーなし。非ブロッカー TODO: 全 test dir を単一 pytest プロセスで一括実行すると conftest.py 名前衝突で collection error（指定どおり suite 別実行なら全 pass、ハーネス起動方法のみの課題）、実 AWS 適用前に dev root への後続配線（各 module 出力→依存 module 入力）・実値注入・Infra_Pipeline 手動承認・moto/testcontainers による統合再検証が必要。_
