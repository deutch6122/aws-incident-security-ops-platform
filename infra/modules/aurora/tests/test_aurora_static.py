...
def test_isolated_subnet_group_and_database_security_group_are_exclusive() -> None:
    ...
    assert re.search(r"subnet_ids\s*=\s*var\.database_subnet_ids", MAIN)
...
def test_tags_and_dev_lifecycle_observability_defaults_are_declared() -> None:
    ...
    assert re.search(r"backup_retention_period\s*=\s*var\.backup_retention_period", MAIN)
    ...
    assert re.search(r"deletion_protection\s*=\s*var\.deletion_protection", MAIN)
    assert re.search(r"skip_final_snapshot\s*=\s*var\.skip_final_snapshot", MAIN)
...
def test_dev_root_wires_network_outputs_and_exposes_non_secret_database_metadata() -> None:
    ...
    assert "module.aurora.app_database_secret_arn" in DEV_MAIN
    assert "module.aurora.app_database_secret_arn" not in DEV_OUTPUTS
