...
def test_api_gateway_origin_is_custom_and_variable_driven() -> None:
    ...
    assert "origin_path" not in block
    api_var = re.search(r'variable "api_gateway_origin_domain" \{(.*?)\n\}', VARIABLES, re.DOTALL)
    assert api_var and "default" not in api_var.group(1)
    assert "length(trimspace(var.api_gateway_origin_domain)) > 0" in api_var.group(1)
...
def test_viewer_protocol_policy_is_https() -> None:
    ...
    assert all(policy == "redirect-to-https" for policy in policies)
    ...
    assert api_beh and re.search(r'path_pattern\s*=\s*"/api/\*"', api_beh.group(1))
    assert re.search(r"target_origin_id\s*=\s*local\.api_origin_id", api_beh.group(1))


def test_api_behavior_uses_aws_managed_no_cache_and_origin_request_policies() -> None:
    block = _resource_block("aws_cloudfront_distribution", "this")
    assert 'name = "Managed-CachingDisabled"' in MAIN
    assert 'name = "Managed-AllViewerExceptHostHeader"' in MAIN
    assert "cache_policy_id          = data.aws_cloudfront_cache_policy.caching_disabled.id" in block
    assert "origin_request_policy_id = data.aws_cloudfront_origin_request_policy.all_viewer_except_host_header.id" in block
    assert '["GET", "HEAD", "OPTIONS", "PUT", "POST", "PATCH", "DELETE"]' in block
...
def test_outputs_publish_distribution_and_oac_identity() -> None:
    for name in (
        "distribution_id",
        "distribution_arn",
